...
OPCODES = {1: Op(add, 2, 1), 2: Op(mul, 2, 1), 3: Op(store, 0, 1), 4: Op(output, 1, 0), 99: Op(end, 0, 0)}


def instruction(op, mode, pgm, start_pos):
    ...
    pgm = pgm.copy()
    inputs = [_get_input(i, val, mode) for i, val in enumerate(pgm[start_pos + 1:start_pos + 1 + op.input_count])]
    result = op.func(*inputs) if op.func in (mul, add) else op.func(pgm, *inputs)
    if op.output_count:
        pgm[pgm[start_pos + 1 + op.input_count]] = result
    else:
        print(result)
    return pgm
...
def run_program(pgm):
    ...
    i = 0
    while True:
        mode, opcode = divmod(pgm[i], 100)
        op = OPCODES[opcode]
        if op.func == end:
            break
        pgm = instruction(op, mode, pgm, i)
        i += 1 + op.input_count + op.output_count
    return pgm
